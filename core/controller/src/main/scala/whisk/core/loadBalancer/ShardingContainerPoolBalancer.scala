...
class ShardingContainerPoolBalancer(config: WhiskConfig, controllerInstance: InstanceId)(
  ...
  /** Factory methods for Throttler*/
  override def throttler(): Throttler = new ShardingContainerPoolThrottler

  /** Loadbalancer methods for Throttling*/
  private def activeActivationsFor(namespace: UUID): Future[Int] =
    Future.successful(activationsPerNamespace.get(namespace).map(_.intValue()).getOrElse(0))
  private def totalActiveActivations: Future[Int] = Future.successful(totalActivations.intValue())
  private def clusterSize: Int = schedulingState.clusterSize

  class ShardingContainerPoolThrottler (implicit logging: Logging) extends Throttler {
    ...
    private def dilateLimit(limit: Int): Int = Math.ceil(limit.toDouble * overcommit(clusterSize)).toInt
    ...
    /**
      ...
      */
    protected def checkUserThrottle(user: Identity, right: Privilege, resources: Set[Resource])(
      implicit transid: TransactionId): Future[Unit] = {
      if (right == ACTIVATE) {
    ...
    /**
      ...
      * @return future completing successfully if user is below limits else failing with a rejection
      */
    protected def checkConcurrentUserThrottle(user: Identity, right: Privilege, resources: Set[Resource])(
      implicit transid: TransactionId): Future[Unit] = {
